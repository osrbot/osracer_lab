...
- `source_authority_snapshot.json`: optional osrcore/osracer source contract snapshot when packaged
...
def main():
    ...
    source_snapshot_out = output_dir / "source_authority_snapshot.json"
    ...
    source_snapshot_path = Path(args.source_authority_snapshot).resolve() if args.source_authority_snapshot else None
    if source_snapshot_path and source_snapshot_path.is_file():
        shutil.copy2(source_snapshot_path, source_snapshot_out)
    else:
    ...
    if source_snapshot_path:
        manifest["artifacts"][source_snapshot_out.name] = {
            "source": str(source_snapshot_path),
            "bytes": source_snapshot_out.stat().st_size,
            "sha256": sha256(source_snapshot_out),
            "optional": True,
            "kind": "source_authority_snapshot",
        }
        manifest["source_authority_snapshot"] = {
            "included": True,
            "artifact": source_snapshot_out.name,
        }
    else:
        manifest["source_authority_snapshot"] = {"included": False}
    if overlay_path:
    ...
    if source_snapshot_path:
        package_files.insert(2, source_snapshot_out)
    if overlay_path:
